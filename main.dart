...
class _HomePageState extends State<HomePage> {
  ...
  // Funcion para mostrar cámara o galería
          ...
          actions: [
            TextButton(
              onPressed: () async {
                ...
                await pickImage(ImageSource.camera);  // Usa camara
              },
              child: Text('Cámara'),
            ),
            TextButton(
              onPressed: () async {
                Navigator.pop(context);
                await pickImage(ImageSource.gallery);  // Usa galeria
              },
              child: Text('Galería'),
            ),
          ],
  ...
  // Funcion para seleccionar la imagen cámara o galeria
  ...
  // Funcion para cargar el modelo de TensorFlow Lite
  ...
  // Funcion para cargar el JSON de medicamentos
  Future<void> loadMedicamentosData() async {
    try {
      String jsonData = await rootBundle.loadString('assets/Medicamentos.json');
      setState(() {
        _medicamentosData = json.decode(jsonData);
      });
      print("Datos de medicamentos cargados correctamente");
    } catch (e) {
      print("Error al cargar los datos de medicamentos: $e");
    }
  }

  // Funcion para clasificar la imagen con TFLite
  Future<void> classifyImage(File image) async {
    ...
    try {
      ...
      int maxIndex = getMaxConfidenceIndex(output[0]); // Obtiene el indice de la predicción con más confianza
      setState(() {
        isLoading = false;
        _predictedLabel = _labels[maxIndex]; // Almacena la etiqueta predicha
        _confidence = output[0][maxIndex]; // Almacena la confianza de la prediccion
      });

      print("Predicción: $_predictedLabel con confianza de $_confidence");

      // Busca el medicamento correspondiente en los datos cargados
      setState(() {
        _medicamentoDetalles = _medicamentosData['medicamentos'].firstWhere(
              (medicamento) => medicamento['nombre'] == _predictedLabel,
          orElse: () => null,
        );
      });
  ...
  // Funcion para cargar las etiquetas desde el archivo labels.txt
  ...
  // Funcion donde se obtiene el indice con mayor confianza de la salida
  int getMaxConfidenceIndex(List<double> output) {
    double maxConfidence = output.reduce((a, b) => a > b ? a : b); // Encuentra el valor maximo
  ...
  @override
  void dispose() {
    _interpreter?.close(); // Cierra el interprete de TensorFlow Lite
    super.dispose();
  }
}